...
- **Where** to patch — a hierarchical path called "scope" (library → cell → pin → group), optionally with **attributes** to disambiguate groups
- **What** to patch — an element-wise matrix addition on the target's `values` attribute

### What are attributes?

In a Liberty `.lib` file, a single pin can contain **multiple groups of the same type**. For example, pin `Y` might have two `timing()` groups — one for `related_pin: "A"` and another for `related_pin: "B"`:
...
Similarly, `internal_power()` groups are distinguished by `when` and `related_pg_pin`:
...
To target a **specific** group, the YAML scope path attaches `attributes` to that group entry. These act as **filters** — Liberty-Patcher only matches groups whose simple attributes (key-value pairs) match all specified values:
...
Common attribute keys: `related_pin`, `when`, `related_pg_pin`, `timing_sense`, `timing_type`.
...
```yaml
modifications:
  # Simple case — no attributes needed (only one timing group, or target all)
  - scope:
      path:
        - library: "*"
  ...
  # With attributes — target only the timing group where related_pin is "A"
  - scope:
      path:
        - library: "*"
        - cell: "AND2x2_ASAP7_6t_SL"
...
4. Any `group` entry may include an `attributes` map to filter by Liberty simple attributes:
   ```yaml
   - group: "timing"
     attributes:
       related_pin: "A"
   ```
   Omit `attributes` when the CSV has no attribute columns or the value is empty.
5. `action` is always exactly:
   ```yaml
   operation: add
   mode: matrix
...
Some CSVs include **attribute columns** that disambiguate groups (see "What are attributes?" above):

```csv
cell,pin,group,related_pin,sub_group,v1,v2,...,v25
AND2x2_ASAP7_6t_SL,Y,timing,A,cell_fall,0.01,0,...,0.01
AND2x2_ASAP7_6t_SL,Y,timing,B,cell_fall,0.005,0,...,0.005
```

Attribute columns appear between the group column and sub_group column. They produce `attributes` on the preceding group entry in the YAML output. If an attribute column's value is empty for a row, omit it from the attributes map.
...
| Attribute columns (`related_pin`, `when`, etc.) | Configurable via `--attribute-columns` CLI arg |
...
| `--attribute-columns` | No | Comma-separated attribute column names (e.g., `related_pin,when`). These columns attach as `attributes` to the preceding `group` path entry. Default: none |
...
3. Determine scope vs. attribute vs. value columns:
   - If --scope-columns given → use those as scope columns
   - If --attribute-columns given → use those as attribute columns
   - Remaining non-scope, non-attribute columns → try float() on first data row to find value columns
4. For each row:
   a. Extract scope values (in order)
   b. Extract attribute values (if any)
   c. Extract numeric values as floats
   d. N = int(sqrt(num_values)) — error if not perfect square
   e. Reshape into N×N 2D list
   f. Build modification:
      scope.path = [{library: <lib>}] + path entries per scope column
        → attach attributes to the preceding group entry if attribute columns have values
...
**Attaching attributes:** When `--attribute-columns` is provided, after building the last `group` entry in the path (before processing subsequent scope columns that also become groups), attach an `attributes` map containing each attribute column's key-value pair. Skip attribute keys whose value is empty in that row.

Example: scope columns `group,related_pin,sub_group` with `--attribute-columns related_pin` produces:
```yaml
- group: "timing"
  attributes:
    related_pin: "A"
...
### Example 2: With attribute columns

#### Input (`margin_with_attrs.csv`):

```csv
cell,pin,group,related_pin,sub_group,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,v13,v14,v15,v16,v17,v18,v19,v20,v21,v22,v23,v24,v25
AND2x2_ASAP7_6t_SL,Y,timing,A,cell_fall,0.01,0,0,0,0,0,0.01,0,0,0,0,0,0.01,0,0,0,0,0,0.01,0,0,0,0,0,0.01
AND2x2_ASAP7_6t_SL,Y,timing,B,cell_fall,0.005,0,0,0,0,0,0.005,0,0,0,0,0,0.005,0,0,0,0,0,0.005,0,0,0,0,0,0.005
```

#### Command:

```bash
python margin_csv_to_yaml.py margin_with_attrs.csv -o margin_patch.yaml --attribute-columns related_pin
```
...
- [ ] `--attribute-columns` attaches attributes to the correct group entry
- [ ] Attribute columns with empty values are omitted from attributes map
